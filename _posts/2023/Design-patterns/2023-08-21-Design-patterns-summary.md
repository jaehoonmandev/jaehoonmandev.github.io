...
이번 포스트에서는 패턴의 상세한 내용은 알아보지 않고 대략 "어떤 패턴이 이런 특징을 가진다." 정도만 설명할 것이다.

## Creation patterns - 생성 패턴

> 시스템을 사용하는데 있어서 객체를 Encapsulating(캡슐화)하고, 인스턴스를 어떻게 생성할 것이고, 어떻게 결합 할지를 결정.
...
##### Singleton pattern
  ...
  전체적인 시스템에서 공통적으로 수행해야하는 동작 객체를 생성할 수 있는 패턴.
...
> 엔티티 간의 관계를 단순하게 맺음으로써 설계를 용이하게 하는 설계 패턴.
...
##### Iterator pattern
  ...
  컨테이너에서 알고리즘 부분을 분리하는 등의 방법을 이용하여 컨테이너의 각 부분을 액세스할 때 사용하는 패턴.

##### Mediator pattern

  직접 연관을 짓는 객체들을 중재자 객체를 통해 캡슐화하여 의존성을 낮추는 패턴.
...
##### Protocol stack
  ...
  여러 개의 레이어를 구성하여 이를 통하여 핸들링하는 패턴.
